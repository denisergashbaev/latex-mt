...
\item To demonstrate that deep features learned by deep CNN can be an appropriate surrogate for hand-crafted feature descriptors. In addition, tzo \todo{what?} synthetically create datasets of images, as realistic as possible, and completely automatically annotated for CNN to train with.
...
\todo{one sentense is a bit meager. you need to relate the objectives to contributions somehow. what obj have been achieved and which not}
...
\begin{enumerate}
	...
	\item It provides a synthetically generated and automatically labeled dataset of pedestrians using unlabeled University of California San Diego(UCSD) pedestrian dataset used in \cite{mahadevan2010anomaly}, to train a counting deep convolutional neural network which is adequate for apprehending the underlying representations of the learned features. To this end, we describe a counting problem for MNIST \todo{what is a MNIST dataset, cite the source} dataset to demonstrate the capability of the internal representation of the network for classifying digits with no direct supervising while training. 
	...
	\item Along with the validation of our proposal in the following ways \todo{this is not a valid sentence in english: subject/verb/object missing}:
	\begin{itemize}
		...
		\item Last but not least\todo{replace this `last but not the least shit' -- use it only in oral presentations}, we count the number of pedestrians in a manually labeled dataset of images provided by~\citealt{chan2013ground}. 
	\end{itemize}
...
In section~\ref{sec:proposal} \todo{do you call them sections or chapters -- be consistent}, we describe our proposal for constructing a deep neural network to tackle feature detection issue learning to count problems. 
...
Section ~\ref{sec:experiments} deals with the empirical experiments and analysis. Obtained results and comparisons drawn to state-of-the-art also is placed in this section \todo{`drawn to' / `placed in' -- wtf, change plz}.  

Lastly, in Chapter~\ref{sec:conclusions} \todo{ok, do you capitalize or not capitalize the word chapter/section -- decide and be consistent. I prefer a capitalized version}, we conclude the report with a short summary of the scope of work conducted and the new areas of research that this master thesis has opened.
