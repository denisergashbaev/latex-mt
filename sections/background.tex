...
%
\subsection{Training algorithm}\todo{so, please capitalize EACH word in ALL sections and subsections}
\label{subsec:sgd}
...
are: \textit{W} \todo{use dollar signs for all variables instead of textit -- i think} -\todo{use two dashes instead: --, it looks better} the weights and biases of the network, (X, y) - the dataset, batch size - the number of training examples that are used in each iteration, and $\alpha$, the learning rate. For notation simplicity, all the model parameters are represented in \textit{W}. In practice, each
layer usually defines a 2-dimensional weight matrix and a 1-dimensional bias vector.

\todo{you should be using the algorithmx package -- look it up in my latex -- to write out an algorithm. leave it as the latest thing}
\begin{figure}[H]
	\centering
...
\todo{you have to quote ALL the picture sources (and actually obtain the permission for their use unless it's from wikipedia)!!!}
\begin{figure}[H]
	\centering
