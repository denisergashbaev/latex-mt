...
\todo{you should use equasion package for equasions. see https://en.wikibooks.org/wiki/LaTeX/Mathematics}
...
\subsubsection{Momentum}\todo{doesn't it belong to state-of-the art / implementation???}
...
\label{imparch}
\todo{this part appears out of nowhere: no transition. also it mentions caffe, which was introduced a while ago. }
...
\noindent We should also mention that at the top layer of the network, we used Euclidean Loss layer to compute  the euclidean distance between the predictions and the ground truth.  Figure~\ref{fig:l2cNet} shows a scheme\todo{`schematic' is an adj} of the architecture.
\begin{figure}[H]
  \centering
...
\todo{shitty image quality like all the other ones. not  possible to read. better increase the resolution/dimensions \& split it in two parts and place in two columns}
\begin{figure}[H]
  \centering
